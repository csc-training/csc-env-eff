...
Same information can be found in [the module section of Docs CSC](https://docs.csc.fi/computing/modules/).
...
- Multiple software with different (possibly conflicting) requirements are needed in a supercomputer
- The solution for managing this situation: separate the applications into *modules*
...
- CSC uses [*Lmod*](https://lmod.readthedocs.io/en/latest/) environment modules which use the *Lua* programming language
...
- These commands will help you navigate the module environment:
   - `module list`: See a list of currently loaded modules
...
- Some modules contain software that have a graphical user interface
   ...
   - Use the [Puhti web interface](https://puhti.csc.fi) or [other connection with GUI](https://docs.csc.fi/computing/connecting/#using-graphical-applications)
...
- A module might contain only one software (*e.g.* `gromacs-env`), while some provide a collection of applications or *e.g.* Python or R packages
...
- Pre-installed Python packages are available in [Python-related modules](https://docs.csc.fi/apps/python/)
   - See Docs CSC for available packages and instructions on usage
...
- If it feels cumbersome to run multiple `module load modulenames` at the start of each session, you can define an *alias* for these in your `.bashrc`
   - Add this line to your `.bashrc`: `alias setmyenv="module load modulenames"`
   - Now you can load all those modules easily with `setmyenv` (after sourcing your `.bashrc` or logging out and in again)
